...
const props = defineProps({
  user: Object,        // { name, email, phone } hoặc null
  products: Array,     // danh sách sản phẩm trong giỏ
  subtotal: Number,
  shipping_fee: Number,
  discount: Number,
  final_total: Number,
...
// Thông tin người đặt (tự động từ user nếu có)
...
// Thông tin người nhận (khởi tạo rỗng)
...
// Khi tick checkbox: copy từ người đặt; khi bỏ tick: clear toàn bộ
watch(sameAsCustomer, (val) => {
  if (val) {
    // Copy thông tin người đặt sang người nhận (giữ nguyên các trường địa chỉ nếu đã có)
    receiverInfo.value.name = customerInfo.value.name
    receiverInfo.value.phone = customerInfo.value.phone
    // Nếu bạn muốn copy luôn email (nếu cần) thì thêm
    // receiverInfo.value.email = customerInfo.value.email
  } else {
    // Clear toàn bộ thông tin người nhận
...
// Tính toán lại subtotal và total từ props
const subtotal = computed(() => props.subtotal || cartItems.value.reduce((sum, i) => sum + i.price * i.quantity, 0))
...
// --- Đặt hàng ---
const placeOrder = () => {
  // Nếu checkbox được tick, đồng bộ lại (đảm bảo)
  ...
  router.post(route('checkout.store'), orderData, {
    onSuccess: () => {
      // Chuyển hướng đến trang thành công (đã có trong controller)
    },
    onError: (errors) => {
