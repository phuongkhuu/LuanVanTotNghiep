    ...
    <main class="max-w-[1440px] mx-auto px-4 md:px-8 py-6 bg-gray-50">
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="md:col-span-5 flex flex-col gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div v-if="product.colors && product.colors.length" class="py-4 border-t border-gray-200">
            ...
            <div class="flex gap-3 flex-wrap">
              <button 
                ...
                :class="selectedColor === color.value ? 'border-primary shadow-lg shadow-primary/30 scale-110' : 'border-gray-200 hover:border-primary hover:scale-105'"
                @click="selectColor(color.value, color.label)"
                :title="color.label"
              >
                <div class="w-full h-full rounded-full" :style="{ backgroundColor: color.code }"></div>
              </button>
            </div>
          </div>
          ...
          <!-- ===== THÔNG BÁO ===== -->
          <div v-if="message" 
               class="p-3 rounded-lg text-sm text-center animate-slide-down"
            ...
            <div class="flex items-center justify-center gap-2">
              <span v-if="messageType === 'success'" class="text-lg">✅</span>
              <span v-else-if="messageType === 'error'" class="text-lg">❌</span>
...
const findVariant = () => {
  ...
  // Ưu tiên tìm theo cả màu và size
  ...
  // Nếu không tìm thấy, tìm theo màu
  if (!found && selectedColor.value) {
    found = variants.find(v => String(v.color_id) === String(selectedColor.value))
  }
  
  // Nếu không tìm thấy, tìm theo size
  if (!found && selectedSize.value) {
    found = variants.find(v => v.size_name === selectedSize.value)
  }
  
  // Nếu vẫn không tìm thấy, lấy variant đầu tiên
  if (!found && variants.length > 0) {
    found = variants[0]
    // Cập nhật màu và size từ variant tìm được
    if (found.color_id) {
      const color = props.product.colors?.find(c => String(c.value) === String(found.color_id))
      if (color) {
        selectedColor.value = color.value
        selectedColorName.value = color.label
      } else {
        selectedColor.value = found.color_id
        selectedColorName.value = found.color_id
      }
    }
    if (found.size_name) {
...
const addToCart = async () => {
  ...
  try {
    ...
    const msg = error.response?.data?.message || 'Quá số lượng tồn kho. Vui lòng thử lại.'
    showMessage(msg, 'error')
  } finally {
    loading.value = false
  }
...
// ===== WATCH ĐỂ CẬP NHẬT KHI THAY ĐỔI =====
watch(selectedColor, (newVal) => {
  if (newVal && props.product.colors) {
    const color = props.product.colors.find(c => String(c.value) === String(newVal))
    if (color) {
      selectedColorName.value = color.label
    }
  }
